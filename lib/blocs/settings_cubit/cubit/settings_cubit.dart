...
// Initialize the settings from the database
  void initSettings() {
    ...
    BloomeeDBService.getSettingStr(GlobalStrConsts.countryCode)
        .then((value) async {
      if (value != null) {
        emit(state.copyWith(countryCode: value));
      } else {
        final detected = await getCountry();
        emit(state.copyWith(countryCode: detected));
      }
    });
    ...
    for (var eg in SourceEngine.values) {
      BloomeeDBService.getSettingBool(eg.value).then((value) {
        final bool fallback = eg == SourceEngine.eng_JIS ? false : true;
        List<bool> switches = List.from(state.sourceEngineSwitches);
        switches[SourceEngine.values.indexOf(eg)] = value ?? fallback;

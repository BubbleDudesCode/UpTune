...
class BloomeeDBService {
  ...
  static Future<void> putRecentlyPlayed(MediaItemDB mediaItemDB) async {
    ...
    if (_mediaItemDB != null) {
      ...
      // Sync to Supabase
      try {
        await SupabaseSyncService.instance.addRecentlyPlayed(
          songId: _mediaItemDB.mediaID,
          title: _mediaItemDB.title,
          artist: _mediaItemDB.artist,
          imageUrl: _mediaItemDB.artURL,
          provider: _mediaItemDB.source,
        );
      } catch (e) {
        log('Failed to sync recently played to cloud: $e', name: 'DB');
      }
    } else {
